...
let inventorNamedAda = inventors.find((inventor => { return inventor.first.includes("Ada") }));
...
let firstLast = people.map((person) => {
    ...
    return personSplit[1] + " " + personSplit[0];
});
...
let isAdultPresent = devs.some((developer => { return developer.year >= 18; }));
...
let isEveryone19OrOlder = devs.every((developer) => { return (2025 - developer.year) >= 19; });
...
let totalYearsLived = inventors.reduce((accumulator,currentValue) => accumulator + (currentValue.passed - currentValue.year), 0);
...
let travelMethodCounts = travelMethods.reduce((accumulator, currentValue) => {
